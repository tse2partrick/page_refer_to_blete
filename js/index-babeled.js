...
let onScroll = function () {
    ...
    setTimeout(() => {
        let { links, offsetTops } = getAnchorsPos();
...
let getAnchorsPos = function () {
    ...
    return { links, offsetTops };
};

// 滚动高亮连接
let scrollActiveLink = function (pos, offsetTops, links) {
    if (pos < offsetTops[0]) {
        $('#nav-bar > li > a').removeClass('active');
        $('#aHome').addClass('active');
    }
